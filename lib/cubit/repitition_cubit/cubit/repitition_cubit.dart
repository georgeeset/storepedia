...
class RepititionCubit extends Cubit<RepititionCubitState> {
  ...
  searchDB({String? partNumber, String? storageLocation, String? storeid}){
    ...
    partQuery.searchPart(searchString: '${storageLocation??''} ${storeid??''}').then((value){
      if(value==null){
    ...
    }).onError((error, stackTrace) { emit(RepititionSearchError(errorMessage: error.toString(),stackTrace: stackTrace));});
  }
