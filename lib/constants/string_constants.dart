library string_constants;
  ...
  const String partNameInfo='This field should contain generic name for the part you are about to add or edit';
  const String partNameTitle='PartName';
  const String partDescriptionTitle='Part Description';
  const String partDescriptionInfo='In your own words List the features you know about this part such as size, dimension, type, class, place of use, material. Eg: M8 stainless steel bolt with countersunk head for conveyor bottle guide';
  const String partNumberTitle='Part Number';
  const String partNumberInfo='Part identification number that can be used to identify the part, it is usually written on the part. You can write more than one, seperated with space';
  const String storeIdTitle='Store Id';
  const String storeIdInfo='This will enable us locate the item in the sparepart store';
  ...
  const String completeYourProfile='Complete your Profile, so that we can tell who you are';
  const String emailVerificationMessage='Your Email is not Yet verified \nClick Here to send Email Verification';
  const String emailVerificationSentMessage='Click here to contiue.\n Your Email Verifcation has been sent to:\n';
  const String errorLoadingProfile='System Failed Loading your profile! check network connection and retry';
  const String noPermisionBody='In order to control App content, Admin Permision is required.\nThis app will be closed\ncontact Admin for support!';
  const String noPermisionTitle='No Admin Permission !';
  const String deviceChanged='Your Device Id has changed. Ensure you are not abusing this app.';
