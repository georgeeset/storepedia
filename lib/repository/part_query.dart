...
class PartQuery{
  ...
  final int maximumSearchResult=10;
  ...
  Future<List<Part>?>searchPart({required String searchString, bool newSearch=false })async{
    //keywords form here...
    ...
    if(newSearch || lastDoc==null){
      queryResult= await _firestoreOperations.queryWithListKeywords(keywords, FirebaseConstants.parts, FirebaseConstants.searchKeywords,maximumSearchResult , FirebaseConstants.likesCount).onError((error, stackTrace) => Future.error(error.toString(),stackTrace),);
    }
    else{
      queryResult= await _firestoreOperations.pagenateQueryWithListKeywords(keywords, FirebaseConstants.parts, FirebaseConstants.searchKeywords,maximumSearchResult , FirebaseConstants.likesCount,lastDoc!).onError((error, stackTrace) => Future.error(error.toString(),stackTrace),);
    }
   ...
   return queryResult.map((data) => Part.fromMap(data)).toList();
  }
